...
def train_valid(model, trainloader, validloader, criterion, n_epochs= wb_config.epochs):
    ...
    for epoch in range(n_epochs):
        ...
        for batch_idx, (data, target) in loop:
            ...
            output = model(data)
            # if batch_idx % random.randint(3,100) == 0:
            #   print(output)
            # batch loss
            loss = criterion(output, target)

            if torch.isnan(loss):
              print("out and tar nan:",output,target)
              raise ValueError(f"loss diverged at {batch_idx}")
            # backward pass
            loss.backward()
        ...
        if train_loss <= train_loss_min:
            ...
            torch.save(model.state_dict(), "/content/gdrive/MyDrive/FHX/weigths/source_model_quant_chess{}.pt".format(epoch))
            train_loss_min = train_loss
