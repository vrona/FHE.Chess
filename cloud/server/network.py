...
class OnDiskNetwork:
    ...
    def run_fhe_inference(self, serialized_qx_new_encrypted, serialized_evaluation_keys, sub_model):
        print("network.py FLAG start inside run")
        server_dir_sub_model = self.server_dir + sub_model
        
        self.fhemodel_server = FHEModelServer(server_dir_sub_model)
        ...
        #print("network.py FLAG serial eval keys gotten")

        #serialized_qx_new_encrypted = self.encrypt_app(chessboard)
        serialized_result = self.fhemodel_server.run(serialized_qx_new_encrypted, serialized_evaluation_keys)
        print(" network.py FLAG end inside run")
