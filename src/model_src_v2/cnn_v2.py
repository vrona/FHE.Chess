...
class PlainChessNET(nn.Module):
    ...
    def __init__(self, hidden_layers=2, hidden_size=768):
        ...
        self.conv_2 = nn.Conv2d(hidden_size, 8, kernel_size=3, stride=1, padding=1)
        self.flatten = nn.Flatten()
        self.to_output = nn.Linear(512,8)

        #self.flatten2 = nn.Linear(768,8) #Conv2d(hidden_size, 8, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.flatten(x)
        x = F.relu(x)
        x = self.to_output(x) # torch.Size([64, 8])
