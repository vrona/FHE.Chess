...
target = torch.empty(16, dtype=torch.long).random_(64)
...
target_p = torch.randn(16, 64).softmax(dim=1)
output_p = loss(input, target_p)
output_p.backward()
print('\nTARGET prob',target_p,'\nOUTPUT prob',output_p)


input_new = torch.tensor([[3.2, 1.3,0.2, 0.8]],dtype=torch.float)
target_new = torch.tensor([0], dtype=torch.long)
loss(input_new, target_new)
print(target_new)
