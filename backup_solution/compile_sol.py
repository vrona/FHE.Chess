...
def compile_brevitas_qat_model(
    ...
    # if len(dummy_input_for_tracing) >1:
    #     dummy_tuple_input = tuple((dummy_input_for_tracing[n] for n in range(len(dummy_input_for_tracing)))) if 
    #     onnx_model = exporter.export(
    #         torch_model,
    #         args = dummy_tuple_input,
    ...
    onnx_model = exporter.export(
        torch_model,
        args= tuple((dummy_input_for_tracing[n] for n in range(len(dummy_input_for_tracing)))) if len(dummy_input_for_tracing) >1 else dummy_input_for_tracing[0],
        export_path=str(output_onnx_file_path),
        keep_initializers_as_inputs=False,
        opset_version=OPSET_VERSION_FOR_ONNX_EXPORT,
    )
